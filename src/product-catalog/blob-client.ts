import { ProductCatalogBlobSchema, type ProductCatalogBlob } from './schema.ts';
...
async function getBlobClient(connStr: string, container: string) {
  ...
  return BlobServiceClient.fromConnectionString(connStr)
    ...
    .getBlockBlobClient('product-catalog.json');
}

const EMPTY_SEED: ProductCatalogBlob = {
  products: [],
};

export async function readProductCatalog(): Promise<ProductCatalogBlob> {
  ...
  return ProductCatalogBlobSchema.parse(JSON.parse(text));
}

export async function writeProductCatalog(data: ProductCatalogBlob): Promise<void> {
  const { connStr, container } = resolveEnv();
  const blobClient = await getBlobClient(connStr, container);
