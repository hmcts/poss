...
<xsl:stylesheet xmlns:ora="http://www.oracle.com/XSL/Transform/java" xmlns:supsfo="http://eds.com/supsfo" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:ns0="xmlns" ns0:xsl="http://www.w3.org/1999/XSL/Transform"><xsl:import href="supsfo.xsl"/><xsl:strip-space elements="*"/><xsl:output method="xml"/><xsl:template match="/"><editableSections><div class="EDITME" id="N100C3"><div><font size="2" face="Times New Roman">
		...
		<div style="margin-bottom: 0.4cm;">
			<span style="font-weight: bold;">IT IS ORDERED THAT </span>this application for a Consolidated Attachment of Earnings Order be <xsl:value-of xmlns="http://eds.com/supsfo" select="$vdCOApplicant"/>
		</div>
		<xsl:if xmlns="http://eds.com/supsfo" test="$vdHearingTime != $emptyTime">
			<div xmlns="" style="margin-bottom: 0.4cm;">
				to <xsl:value-of xmlns="http://eds.com/supsfo" select="$vdHearingDate"/>
			</div>
			<div xmlns="" style="margin-bottom: 0.4cm;">
				at <xsl:value-of xmlns="http://eds.com/supsfo" select="$vdHearingTime"/>
			</div>
			<div xmlns="" style="margin-bottom: 0.4cm;">
				at <xsl:value-of xmlns="http://eds.com/supsfo" select="$vdOrderCourtAddress"/>
			</div>
		</xsl:if>
