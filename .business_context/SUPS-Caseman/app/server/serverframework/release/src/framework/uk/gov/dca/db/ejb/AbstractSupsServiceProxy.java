...
{
	private static final Log log = SUPSLogFactory.getLogger(AbstractSupsServiceProxy.class);

    protected final static Class[] LOCAL_SERVICE_PARAM_TYPES = { String.class, Writer.class };
    ...
    public org.jdom.Document getJDOM(String jndiName, String methodName, String xmlParams)
    	throws SystemException,BusinessException
    {
        String response = getString(jndiName, methodName, xmlParams, true);
        ...
        if ( response != null && response.length() > 0)
        {
	        try {
	        	...
	        	throw new SystemException("Failed to load results of service call '"+jndiName+"."+methodName+"', result string '"+response+"' : "+e.getMessage(),e);
	        }
	        catch(JDOMException e) {
	        	log.error("Failed to process results of service call '"+jndiName+"."+methodName+"', result string '"+response+"' : "+e.getMessage(),e);
	        }
        }
