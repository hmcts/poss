...
/**
 * A component used to queue a report request to the report server. This version of the service is identical to the
 * ReportInvokerService except for the location of the name of the queue to which the report should be sent. This service 
 * expects to find the information in the set of parameters supplied by the caller rather than in the processing instructions
 * directly. See below for more detail on the expected configuration: -
 * 
 * Expected config of form:
 * 
 * <InvokeReport project="" xpath="/params/param[@name='']" destination="/params/param[@name='destination']">
 * 		<ReportStore id=""/>
 * </InvokeReport>
 * 
 * 
 ...
 *
 */
public class ReportInvokerConfigurableQueueService extends AbstractComponent2 implements IGenerator {
	...
	private String m_destinationXPath;
	private String m_inputXPath;
	private DataSource m_reportQueueStore;
	private QueueHandler m_queueHandler;
	private Map localPreloadCache; // used to temporarily store the preload cache 
	...
	protected void process() throws BusinessException, SystemException {
		...
		// read the destination from the input params
		try {
	        Element m_destinationElement = (Element)XPath.selectSingleNode(parameters, m_destinationXPath);
	        m_destination = m_destinationElement.getText();
		}
		catch(JDOMException e) {
			throw new BusinessException("Unable to find destination attribute using XPath '"+m_destinationXPath+"'");
		}
		...
		if (m_destination != null && m_destination.length() > 0) {
			m_queueHandler = (QueueHandler)localPreloadCache.get(m_destination);
			localPreloadCache = null; // don't hold onto this any longer than necessary
		}
		
		if ( m_queueHandler == null ) {
			throw new BusinessException("Unable to find queue handler with name '"+m_destination+"'");
		}
	...
	private void readProcessingInstructions(Element processingInstructions, Map preloadCache)
		throws SystemException
	{
		// temporarily store the preload cache to allow access to the queue handler contained within it at process time
		localPreloadCache = preloadCache;
		...
		m_destinationXPath = processingInstructions.getAttributeValue(DESTINATION_ATTRIBUTE);
		...
		throws ConfigurationException, QueryEngineException
	{
		...
		if ( m_destinationXPath == null || m_destinationXPath.length() == 0) {
			raiseError(preface+"'"+DESTINATION_ATTRIBUTE+"' attribute required by '"+this.getName()+"'", handler);
		}
