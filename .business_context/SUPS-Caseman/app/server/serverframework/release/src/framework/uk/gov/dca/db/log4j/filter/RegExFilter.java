...
/**
 * This Filter is only to change the password logged by some projects to
 * asteriks ****** :
 * 
 * @author Peter Neil
 * 
...
public class RegExFilter extends Filter {
	...
	private static final Pattern DEFAULT_SEARCH_REG_EX_PATTERN = Pattern
			.compile(DEFAULT_SEARCH_REG_EX, Pattern.CASE_INSENSITIVE
					| Pattern.MULTILINE);
	...
	private Level levelToMatch = null;

	private String filterIndexes = "2"; // comma separated list of matches to be
										// "filtered"

	private String filterString = "'filtered'"; // comma seperated list of
												// strings to use to filter out
												// the matches
	...
	public int decide(LoggingEvent logEvent) {
		...
		if (levelToMatch == null){
			levelToMatch = OptionConverter.toLevel("ERROR", null);
		}
		if(logEvent.getLevel().equals(levelToMatch)){
			found = doFind(logEvent);
	...
	private boolean doFind(LoggingEvent logEvent) {
        if(logEvent == null)
        {
            return false;
        }
		...
		Matcher matches = searchPattern.matcher(logEvent.getMessage()
				.toString());
		if (matches.find()) {
			return true;
		}
	...
	public static void main(String[] args) {
		String searchFor = "Login Failed for user: 'fzzjsd', pass: 'Password01'. Reason: Logon failure: unknown user name or bad password.";
		RegExFilter spf = new RegExFilter();
		/**
		 * LoggingEvent(String fqnOfCategoryClass, Category category, Priority
		 * priority, Object message, Throwable throwable)
		 */
		LoggingEvent logEvent = new LoggingEvent("fqnOfCategoryClass", Logger
				.getInstance(RegExFilter.class), Priority.ERROR, searchFor,
				new Exception("The Exception"));
		System.out.println("Result: " + spf.doReplace(logEvent));

		System.out.println("accept: " + Filter.ACCEPT + ", deny: "
				+ Filter.DENY);
	...
	private String doReplace(LoggingEvent logEvent) {
		...
		for (int i = 0; i < misses.length; i++) {
			// you can use a single replace string or there must be one for
		...
		String result = "";
		System.out.println("Looking in: " + logEvent.getMessage().toString());
		Matcher matches = searchPattern.matcher(logEvent.getMessage()
				.toString());
		if (matches.find()) {
			...
			for (int f = 1; f <= matches.groupCount(); f++) {
				...
				System.out
						.println("key: " + f + " replaceWith: " + replaceWith);
				if (replaceWith != null) {
					result += replaceWith;
				} else {
	...
	public void setSearchRegEx(String searchRegEx) {
		...
		searchPattern = Pattern.compile(searchRegEx, Pattern.CASE_INSENSITIVE
				| Pattern.MULTILINE);
	}
