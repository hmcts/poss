 ...
 * @author Grant Miller
 */
public class TransformService extends AbstractComponent2 implements IGenerator {

	private static final Log log = SUPSLogFactory.getLogger(TransformService.class);
     ...
     */
    public void process() throws SystemException, BusinessException
	{    	
        log.debug("Loading template: " + m_sXsltTemplate);
        
        StringReader reader = new StringReader((String)this.m_inputData.getData(String.class));
    	InputSource xsltSrc = Util.getInputSource(m_sXsltTemplate, this);
    	...
    	if (m_srcXPath == null)
    	{
    		...
    		try {
    			theTransformer.transform(xsltSrc, new InputSource(reader), out);
    		}
    	...
    	else
    	{
			...
			SUPSTransformer theTransformer = new SUPSTransformer();
    		try {
    			theTransformer.transform(new SAXSource(xsltSrc) , new JDOMSource(eSrcXML), out);
    		}
    		catch(TransformerException e) {
