...
public class SUPSService implements XMLService {
    ...
    private static Log log = SUPSLogFactory.getLogger(SUPSService.class);
    ...
    // used to create converters to handle data format conversion between pipeline components 
    //private ConverterFactory m_converterFactory = new ConverterFactory();
    ...
    public void validate(QueryEngineErrorHandler handler) throws SystemException
    {
	    ...
	    context.putSystemItem(IComponentContext.SERVICE_NAME_KEY, m_serviceName);
    	...
    	for( int i = 0; i < m_methods.length; i++ )
    	{
    		...
    		if ( methodSrc != null)
    		{
	        	...
	        	context.putSystemItem(IComponentContext.METHOD_NAME_KEY, getMethodName(methodId) );
    ...
    private void registerTemplate(String templateName) throws SystemException
	{
	    ...
	    ComponentContext context = new ComponentContext();
	    context.putSystemItem(IComponentContext.SERVICE_NAME_KEY, m_serviceName);
        context.putSystemItem(IComponentContext.METHOD_NAME_KEY, methodName);
        ...
        if(log.isDebugEnabled()){
            log.debug("Added template: " + methodName);
        }
    }
    	...
    	throws BusinessException, SystemException
	{
        ...
        if (context == null) {
            throw new SystemException("The current call stack context is null");
        }
    ...
    public Document applyFilter(String userId, String message) throws BusinessException, SystemException {
    	...
    	try {    		
			...
			resultDoc = serviceFilter.apply(userId, doc);
		} 
    	catch(JDOMException e) {
			throw new SystemException("Could not parse message for filters:" + e.getMessage(), e);
		} 
    	catch(IOException e) {
			throw new SystemException("Could not parse message for filters:" + e.getMessage(), e);
		}
    ...
    protected void cacheLookupItem(Map cacheMap, Element eCfg) throws SystemException
    {
    	if ( eCfg != null )
    	{
    		...
    		if ( sClass != null ){
    			try {
    				Class itemClass = ClassUtil.loadClass( sClass );
    				...
    				try {
    					...
    					if(sId.equals(APPLICATION_NAME))
    						log = SUPSLogFactory.getLogger((String) oItem, this.getClass()); 
    				}
    				catch( ConfigurationItemException e ) {
    					throw new ConfigurationException("Failed to retrieve configuration item: "+e.getMessage() ,e);
    				}
    			}
