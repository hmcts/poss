 ...
 * Window - Preferences - Java - Code Style - Code Templates
 */
public class ObligationsProcessor extends AbstractXMLProcessor {
    private static final Log log = LogFactory.getLog(ObligationsProcessor.class);
    private static final String START_TAG = "<params><param name=\"obligation\">"; //<Obligations>";
    private static final String END_TAG = "</param></params>";
    
    public ObligationsProcessor(IServiceType serviceType) {
        super(serviceType);
    }
    ...
    public void xmlProcessor(List params, Map keySeq, Document doc) throws SystemException {
        ...
        if(params.size() > 0) {
	        ...
	        addEndTag(strBuf);
	        obligationStr = strBuf.toString();
	        obligationStr = replaceKeys(obligationStr, keySeq, doc, IXMLProcessor.CASEEVENT_KEY, "<EventSeq>", "//params//ManageCase//Obligations//Obligation//EventSeq");
	        obligationStr = replaceKeys(obligationStr, keySeq, doc, IXMLProcessor.AEEVENT_KEY, "<AEEventSeq>", "//params//ManageCase//Obligations//Obligation//AEEventSeq");
	        callService(obligationStr);
        }
