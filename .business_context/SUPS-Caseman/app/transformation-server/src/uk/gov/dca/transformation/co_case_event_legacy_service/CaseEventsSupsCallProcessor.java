 ...
 */
package uk.gov.dca.transformation.co_case_event_legacy_service;
 ...
 * Window - Preferences - Java - Code Style - Code Templates
 */
public class CaseEventsSupsCallProcessor implements ICustomProcessor {
    ...
    public void process(Document params, Writer writer, Log log) throws SystemException
	{
		...
		try {
		    String xml = WebServiceFactory.createWebServiceDelagator(IWebServiceDelegator.CASE_EVENT_SERVER_NAME, IWebServiceDelegator.CASE_EVENT_METHOD_NAME).invokeWebService(strBuf.toString());
			log.info("CaseEventsSupsCallProcessor -- XML = " + xml);

	    } catch(Exception e) {
	        throw new SystemException("Exception caught in CaseEventsSupsCallProcessor = " + e.getMessage());
	    }
	}
