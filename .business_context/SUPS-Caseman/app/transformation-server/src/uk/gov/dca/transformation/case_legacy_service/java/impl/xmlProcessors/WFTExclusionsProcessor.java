 ...
 * Window - Preferences - Java - Code Style - Code Templates
 */
public class WFTExclusionsProcessor extends AbstractXMLProcessor {
    private static final Log log = LogFactory.getLog(WFTExclusionsProcessor.class);
    private static final String START_TAG = "<params><param name=\"WFTExclusion\">"; 
    private static final String END_TAG = "</param></params>";
    
    public WFTExclusionsProcessor(IServiceType serviceType) {
        super(serviceType);
    }
    ...
    public void xmlProcessor(List params, Map keySeq, Document doc) throws SystemException {
        ...
        if(params.size() > 0) {
	        ...
	        addEndTag(strBuf);
	        WFTExclusionsStr = strBuf.toString();
	        callService(WFTExclusionsStr);
        }
