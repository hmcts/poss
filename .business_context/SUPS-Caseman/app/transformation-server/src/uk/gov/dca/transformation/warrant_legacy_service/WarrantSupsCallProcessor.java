 ...
 */
package uk.gov.dca.transformation.warrant_legacy_service;
 ...
 * Window - Preferences - Java - Code Style - Code Templates
 */
public class WarrantSupsCallProcessor implements ICustomProcessor {
    ...
    public void process(Document params, Writer writer, Log log) throws SystemException
	{
        ...
        try {
            ((Element) XPath.selectSingleNode(params, "/ds/Warrant/OwnedBy")).setText(((Element)XPath.selectSingleNode(params, "/ds/Warrant/ExecutingCourtCode")).getText());
        } catch (JDOMException e) {
            throw new SystemException("JDOM Exception caught in WarrantSupsCallProcessor = " + e.getMessage());
        } 
		...
		try {
		    String xml = WebServiceFactory.createWebServiceDelagator(IWebServiceDelegator.WARRANT_SERVER_NAME, IWebServiceDelegator.WARRANT_METHOD_NAME).invokeWebService(strBuf.toString());
			log.info("WarrantTransformationCallProcessor -- XML = " + xml);

	    } catch(Exception e) {
	        throw new SystemException("Exception caught in WarrantSupsCallProcessor = " + e.getMessage());
	    }
	}
