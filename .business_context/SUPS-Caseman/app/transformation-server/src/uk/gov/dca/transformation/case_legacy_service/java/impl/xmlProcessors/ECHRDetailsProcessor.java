 ...
 * Window - Preferences - Java - Code Style - Code Templates
 */
public class ECHRDetailsProcessor extends AbstractXMLProcessor {
    private static final Log log = LogFactory.getLog(ECHRDetailsProcessor.class);
    private static final String START_TAG = "<params><param name=\"ECHRDetail\">"; 
    private static final String END_TAG = "</param></params>";
    
    public ECHRDetailsProcessor(IServiceType serviceType) {
        super(serviceType);
    }
    ...
    public void xmlProcessor(List params, Map keySeq, Document doc) throws SystemException {
        ...
        if(params.size() > 0) {
	        ...
	        addEndTag(strBuf);
	        ECHRDetailStr = strBuf.toString();
	        ECHRDetailStr = replaceKeys(ECHRDetailStr, keySeq, doc, IXMLProcessor.CASEEVENT_KEY, "<EventSeq>", "//params//ManageCase//ECHRDetails//ECHRDetail//EventSeq");
	        callService(ECHRDetailStr);
        }
