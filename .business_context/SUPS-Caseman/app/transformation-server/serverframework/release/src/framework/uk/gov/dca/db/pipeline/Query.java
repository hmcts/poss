...
public class Query {
    ...
    protected static final Pattern s_varContentPattern = Pattern.compile("\\b(\\w+)\\.(\\w+)\\b");
    ...
    */
    ...
    Object addContextParameter( ContextVariables contextVariables, String parameter, Object value) 
	{
        //convert value here
    ...
    protected class ExistanceCheck 
	{
        ...
        private int m_onExistsAction;
        private int m_onNotExistsAction;
        ...
        protected void setOnExistsAction(String onExistsAction) {
            // defaults to update
        	int action = validateExistsAction(onExistsAction);
        	if ( action == INVALID )
        		m_onExistsAction = UPDATE;
        	else
        		m_onExistsAction = action;
        }

        protected void setOnNotExistsAction(String onNotExistsAction) {
        	// defaults to add
        	int action = validateExistsAction(onNotExistsAction);
        	if ( action == INVALID )
        		m_onNotExistsAction = ADD;
        	else
        		m_onNotExistsAction = action;
        }

        protected int validateExistsAction(String sAction) {
            int action = INVALID;
        ...
        public int getOnExistsAction() {
            return m_onExistsAction;
        }

        public int getOnNotExistsAction() {
    ...
    private boolean shouldAddVariable(String variable) {
        if (m_parameters.contains(variable)) {
            return false;
        }
     ...
     protected void prepareTypes(Connection connection) throws SystemException {
    	...
    	try {
    		while (i.hasNext()) {
    			...
    			try {
    				rs = connection.getMetaData().getColumns(null, null, table.getName(), null);
    			}
                catch(SQLException e) {
                	throw new SystemException("Failed to get database metadata for table '"+table.getName()+"': "+e.getMessage(),e);
                }
                
        		Iterator properties = m_modifyVariables.getVariablesFor(table.getAlias()).iterator();
        		try {
        			while (rs.next()) {
        				String pName = table.getAlias() + "." + rs.getString(4).toUpperCase();
