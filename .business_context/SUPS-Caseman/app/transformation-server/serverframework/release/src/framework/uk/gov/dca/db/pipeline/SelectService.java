...
public class SelectService extends DBService {
    ...
    private static final Log log = LogFactory.getLog(SelectService.class);
        ...
        while (queries.hasNext()) {
			...
			if(selectStatement == null) {
				...
				if ( query.hasDynamicSelect() == false) {
					...
					try {
						addStatement(query.getPivotNode(), m_connection.prepareStatement(buff.toString()));
					}
					catch(SQLException e) {
						throw new SystemException("Failed to prepare statement for '"+query.getPivotNode()+"': "+e.getMessage()+e);
					}
				}
			}
        }
    ...
    protected void processTemplate(Document parameters) throws BusinessException, SystemException 
	{
        ...
        try {
        	...
        	throw new SystemException("Failed to write to data sink: "+e.getMessage(),e);
        }
        ...
        processNode(contextVariables, 0, "/" + element.getName(), element);	
    }

    private void processNode(ContextVariables contextVariables, int level, String currentPath, Element element) 
    	throws SystemException, BusinessException
	{
        /*
    		...
    		if (m_queryDefinitions.m_queries.containsKey(currentPath) || sqlStatement != null) {
	            ...
	            try {
		            while (rs.next()) {   	   	
		                ...
		                writeElement(newContextVariables, level, currentPath, element, rs, query);
		            }
	            }
	            catch(SQLException e) {
	            ...
	            writeElement(contextVariables, level, currentPath, element, rs, query);
	        }
    	}
    	finally {
    		try {
    	...
    	throws SystemException, BusinessException
	{
    	try {
	        writeIndent(level);
	        m_dataSink.write("<");
	        m_dataSink.write(element.getName());
	        Iterator attributes = element.getAttributes().iterator();
	        while (attributes.hasNext()) {
	            Attribute attribute = (Attribute) attributes.next();
	            m_dataSink.write(" ");
	            m_dataSink.write(attribute.getName());
	            m_dataSink.write("=\"");
	            writeParameters(contextVariables, query, attribute.getValue(), true);
	            m_dataSink.write("\"");
	        }
	        m_dataSink.write(">");
	        ...
	        if ( query != null && query.isModifiable() == true ) {
		    	...
		    	while( it.hasNext() ) {
		    		...
		    		if ( oSCN != null ) {
						...
						m_dataSink.write("\r\n");
						writeIndent(level+1);
			    		m_dataSink.write("<SCN table=\"" + table.getAlias() + "\">" + sSCN + "</SCN>");
		    		}			
		    	} 
	        }
	        ...
	        if (hasElements) {
	            ...
	            while (i.hasNext()) {
	                ...
	                processNode(contextVariables, level + 1, currentPath + "/" + newElement.getName(), newElement);
	            }
	        } else {
	            Iterator text = element.getContent(s_textFilter).iterator();
	            while (text.hasNext()) {
	                writeParameters(contextVariables, query, ((Text) text.next()).getText(), false);
	            }
	        }
	        if (hasElements) {
	            writeIndent(level);
    ...
    private void writeParameters(ContextVariables contextVariables, Query query, String text, boolean isAttribute)
    	throws SystemException
	{
        ...
        try {
	        while (vars.find()) {
	            if (lastEnd != vars.start()) {
	                m_dataSink.write(text.substring(lastEnd, vars.start()));
	            }
	            lastEnd = vars.end() + 1;
	            String variable = vars.group(1).toUpperCase();
	            Object result = contextVariables.getValue(variable);
	            String sResult = "";
	            if ( result != null) sResult = result.toString();
	            
	            m_dataSink.write( sResult );
	        }
        }
        catch(IOException e){
        	throw new SystemException("Failed to write to pipeline component output: "+e.getMessage(),e);
