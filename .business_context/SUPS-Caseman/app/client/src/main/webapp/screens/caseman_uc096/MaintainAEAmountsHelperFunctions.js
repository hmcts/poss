/** 
 * @fileoverview MaintainAEAmountsHelperFunctions.js:
 * This file contains the helper functions for UC096 - Maintain AE Refunds and Fees screen.
 *
...
function exitScreen()
{
	Services.removeNode(MaintainAEAmountParams.PARENT);
...
function retrieveCaseData()
{
	...
	Services.callService("getAeAmounts", params, Header_CaseNumber, true);
}
...
{
	// Use the default currency brought in via Reference Data if blank.
...
function feeCreatedToday()
{
	...
	var feeExists = Services.exists(XPathConstants.DATA_XPATH + "/AEAmounts/AEAmount[./Type = 'FEE' and ./DateCreated = '" + today + "' and ./Deleted != 'Y']");
	return feeExists;
}
...
{
	var today = CaseManUtils.getSystemDate(XPathConstants.SYSTEMDATE_XPATH);
	var refundExists = Services.exists(XPathConstants.DATA_XPATH + "/AEAmounts/AEAmount[./Type = 'REFUND' and ./DateCreated = '" + today + "' and ./Deleted != 'Y']");
	return refundExists;
}
...
function calculateTotalFees()
{
	...
	for (var i=0; i<Services.countNodes(XPathConstants.DATA_XPATH + "/AEAmounts/AEAmount"); i++)
	{
		if ( Services.getValue(XPathConstants.DATA_XPATH + "/AEAmounts/AEAmount[" + (i+1) + "]/Deleted") != "Y" )
		{
			tempAmount = Services.getValue(XPathConstants.DATA_XPATH + "/AEAmounts/AEAmount[" + (i+1) + "]/Amount");
	...
	for (var i=0; i<Services.countNodes(XPathConstants.NEWAMOUNTS_XPATH + "/AEAmounts/AEAmount"); i++)
	{
		tempAmount = Services.getValue(XPathConstants.NEWAMOUNTS_XPATH + "/AEAmounts/AEAmount[" + (i+1) + "]/Amount");
		if ( null == CaseManValidationHelper.validatePattern(tempAmount, CaseManValidationHelper.CURRENCY_PATTERN, "CaseMan_invalidAmountFormat_Msg") )
		{
			// Add fee if it is valid amount
			totalFees = totalFees + parseFloat(tempAmount);
...
function generateNextSurrogateId()
{
	...
	if ( 0 == countNewAmounts )
	{
		...
		var xpathRoot = XPathConstants.DATA_XPATH + "/AEAmounts/AEAmount";
	}
	else
	{
		// Get the latest surrogate id from the new amounts grid data source
		var xpathRoot = XPathConstants.NEWAMOUNTS_XPATH + "/AEAmounts/AEAmount";
	}
