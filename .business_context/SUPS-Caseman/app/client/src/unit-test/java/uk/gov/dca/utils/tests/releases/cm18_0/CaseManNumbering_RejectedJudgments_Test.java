...
import uk.gov.dca.utils.screens.CCBCUC019RejectedJudgmentsUtils;
import uk.gov.dca.utils.velo.db.DBUtil;

/**
 * Automated tests for the Case Numbering changes to the View Rejected Judgments screen.
 *
 * @author Chris Vincent
 */
public class CaseManNumbering_RejectedJudgments_Test extends AbstractCmTestBase
{
    
    /** The my CCBCUC 019 rejected judgments utils. */
    // Private member variables for the screen utils
    private CCBCUC019RejectedJudgmentsUtils myCCBCUC019RejectedJudgmentsUtils;

    /**
     * Constructor.
     */
    public CaseManNumbering_RejectedJudgments_Test ()
    {
        super (CaseManNumbering_RejectedJudgments_Test.class.getName ());
        this.nav = new Navigator (this);
        myCCBCUC019RejectedJudgmentsUtils = new CCBCUC019RejectedJudgmentsUtils (this);
    }

    /**
     * Checks that Case Number field validation is as expected on the View Rejected Judgments screen
    ...
    public void testCaseNumberValidationPreChange ()
    {
        try
        {
            ...
            // Get to View Rejected Judgments screen
            ...
            {
                myCCBCUC019RejectedJudgmentsUtils.setCaseNumber (invalidCaseArray[i]);
                assertFalse ("Case Number is valid when should be invalid",
                        myCCBCUC019RejectedJudgmentsUtils.isCaseNumberValid ());
                myCCBCUC019RejectedJudgmentsUtils.setCaseNumberFocus ();
                mCheckStatusBarText (CCBCUC019RejectedJudgmentsUtils.ERR_MSG_INVALID_CASE_NUMBER);
            }
            ...
            {
                myCCBCUC019RejectedJudgmentsUtils.setCaseNumber (invalidCaseArray2[i]);
                assertFalse ("Case Number is valid when should be invalid",
                        myCCBCUC019RejectedJudgmentsUtils.isCaseNumberValid ());
                myCCBCUC019RejectedJudgmentsUtils.setCaseNumberFocus ();
                mCheckStatusBarText (CCBCUC019RejectedJudgmentsUtils.ERR_MSG_INVALID_CASE_LENGTH);
            }
            ...
            {
                myCCBCUC019RejectedJudgmentsUtils.setCaseNumber (validCaseArray[i]);
                assertTrue ("Case Number is invalid when should be valid",
                        myCCBCUC019RejectedJudgmentsUtils.isCaseNumberValid ());
            }

            // Close Screen
            myCCBCUC019RejectedJudgmentsUtils.closeScreen ();
        }
    ...
    /**
     * Private function that logs the user into CaseMan and navigates to the View Rejected Judgments screen.
     */
    private void mLoginAndNavigateToScreen ()
    {
        ...
        // Navigate to the View Rejected Judgments screen
        this.nav.navigateFromMainMenu (MAINMENU_REJECTED_JUDGMENTS_PATH);

        // Check in correct screen
        mCheckPageTitle (myCCBCUC019RejectedJudgmentsUtils.getScreenTitle ());
    }
