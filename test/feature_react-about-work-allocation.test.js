...
const KNOWN_KEYS = ['whatItDoes', 'alignmentCategories', 'scopeAssumption', 'byContextAssumption'];
...
describe('getAboutSection — alignmentCategories section', () => {
  it('A-3: heading is a non-empty string', () => {
    const section = getAboutSection('alignmentCategories');
    assert.ok(section, 'Section must exist');
    assert.ok(typeof section.heading === 'string' && section.heading.length > 0, 'heading must be non-empty');
  });
...
describe('getAboutSection — scopeAssumption section', () => {
  it('A-5: heading is a non-empty string', () => {
    const section = getAboutSection('scopeAssumption');
    assert.ok(section, 'Section must exist');
    assert.ok(typeof section.heading === 'string' && section.heading.length > 0, 'heading must be non-empty');
  });
...
describe('getAboutSection — byContextAssumption section', () => {
  it('A-7: heading is a non-empty string', () => {
    const section = getAboutSection('byContextAssumption');
    assert.ok(section, 'Section must exist');
    assert.ok(typeof section.heading === 'string' && section.heading.length > 0, 'heading must be non-empty');
  });
...
describe('getAboutSections — ordered list', () => {
  it('C-1: returns array of 4 section objects', () => {
    const sections = getAboutSections();
    assert.ok(Array.isArray(sections), 'Must return an array');
    assert.equal(sections.length, 4, 'Must have exactly 4 sections');
  });
